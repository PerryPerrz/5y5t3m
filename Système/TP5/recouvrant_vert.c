...
int main(int argc, char *argv[])
{
    ...
    //On s'occupe des niveaux de vert.
    ...
    for (int i = 0; i < 9; ++i) //On veut aller jusqu'à la 12éme ligne du tableau (comprise)
    {
        switch (i)
        {
        case 2: //On récupère l'offset de l'image (utilise pour les niveaux de vert)
    ...
    unsigned char pixel_b, pixel_v, pixel_r, nouveau_pixel_b, nouveau_pixel_r;
    for (int j = 0; j < hauteur; ++j) //On parcours les lignes en hauteur
    {
        for (int i = 0; i < largeur; ++i) //On s'occupe des pixels d'une ligne de la bitmap
        {
            ...
            //On les mets dans la copie verte
            write(desc3, &nouveau_pixel_b, sizeof(unsigned char));
            write(desc3, &pixel_v, sizeof(unsigned char));
    ...
    printf("La copie verte du fichier à bien été réalisée !\n");
    close(argv[2][0]);
    close(desc3);
