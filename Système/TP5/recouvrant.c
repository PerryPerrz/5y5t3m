...
int main(int argc, char *argv[])
{
    ...
    //On s'occupe des niveaux de gris.
    ...
    unsigned char pixel_gris, pixel_b, pixel_v, pixel_r, nouveau_pixel_b, nouveau_pixel_v, nouveau_pixel_r;
    for (int j = 0; j < hauteur; ++j) //On parcours les lignes en hauteur
    {
        for (int i = 0; i < largeur; ++i) //On s'occupe des pixels d'une ligne de la bitmap
        {
            ...
            //On les mets en gris
            pixel_gris = (char)(pixel_r * .299 + pixel_v * .587 + pixel_b * .114);
            nouveau_pixel_b = pixel_gris;
            nouveau_pixel_v = pixel_gris;
            nouveau_pixel_r = pixel_gris;

            //On les mets dans la copie grise
    ...
    printf("La copie grise du fichier à bien été réalisée !\n");
    close(argv[2][0]);
    close(desc3);
