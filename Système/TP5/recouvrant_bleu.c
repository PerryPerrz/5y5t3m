...
int main(int argc, char *argv[])
{
    ...
    //On s'occupe des niveaux de bleu.
    ...
    for (int i = 0; i < 9; ++i) //On veut aller jusqu'à la 12éme ligne du tableau (comprise)
    {
        switch (i)
        {
        case 2: //On récupère l'offset de l'image (utilise pour les niveaux de bleu)
    ...
    unsigned char pixel_b, pixel_v, pixel_r, nouveau_pixel_v, nouveau_pixel_r;
    for (int j = 0; j < hauteur; ++j) //On parcours les lignes en hauteur
    {
        for (int i = 0; i < largeur; ++i) //On s'occupe des pixels d'une ligne de la bitmap
        {
            ...
            //On les mets en bleu
            //On enlève les nuances de vert et de rouge, on garde seulement celle de bleu.
            nouveau_pixel_v = 0;
            nouveau_pixel_r = 0;

            //On les mets dans la copie bleue
            write(desc3, &pixel_b, sizeof(unsigned char));
            write(desc3, &nouveau_pixel_v, sizeof(unsigned char));
            write(desc3, &nouveau_pixel_r, sizeof(unsigned char));
        }
        /*
        ...
        {
            if( write(desc3, &tp, sizeof(unsigned char)) == -1){
                perror("OK");
                exit(1);
            }
        }
    ...
    printf("La copie bleue du fichier à bien été réalisée !\n");
    close(argv[2][0]);
    close(desc3);
